    ...
    public unsafe class DX9RenderInterface : IRenderInterface {
        ...
        public void Render2D() {
            using (var stateBlock = new StateBlock(D3Ddevice, StateBlockType.All)) {
                stateBlock.Capture();
#if NETFRAMEWORK
                var projection = Matrix4x4.CreateOrthographicOffCenter(0, D3Ddevice.Viewport.Width, D3Ddevice.Viewport.Height, 0, -1000, 1000).ToDX();
#else
                var projection = Matrix4x4.CreateOrthographicOffCenterLeftHanded(0, D3Ddevice.Viewport.Width, D3Ddevice.Viewport.Height, 0, -1, 1).ToDX();
#endif
