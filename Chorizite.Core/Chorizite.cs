    ...
    public class Chorizite<TBackend> : IDisposable where TBackend : IChoriziteBackend {
        ...
        public Chorizite(IChoriziteConfig config) {
            ...
            ChoriziteStatics.Scope = _container.BeginLifetimeScope(builder => {
                ...
                builder.RegisterInstance(new PluginManager(Config.Environment, Config.PluginDirectory, config.StorageDirectory, Scope, Backend.Renderer, ChoriziteStatics.MakeLogger("PluginManager")))
                    .As<IPluginManager>()
                    .SingleInstance();
