    ...
    public class VirtualNode {
        ...
        internal void UpdateElement(Element element) {
            ...
            foreach (var prop in Props) {
                if (prop.Key.StartsWith("on") && prop.Value is Action<Event> action) {
                    CoreUIPlugin.Log.LogDebug($"Adding event listener {prop.Key} to {element.TagName}");
                    element.AddEventListener(prop.Key.Substring(2), action);
                }
                else {
